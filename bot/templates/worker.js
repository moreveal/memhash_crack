...
let currentEnergy = 0, maxEnergy = 1;
...
self.onmessage = function (event) {
  ...
  } else if (data.current_energy !== undefined && data.max_energy !== undefined) {
...
function bigintToUint8Array(value, byteLength = Math.ceil(value.toString(16).length / 2), pad = 32, littleEndian = false) {
  ...
  for (let i = 0; i < byteLength; i++) {
    const byte = Number(value & BigInt(0xff));
    array[littleEndian ? i : byteLength - 1 - i] = byte;
    value >>= BigInt(8);
  }
...
function uint8ArrayToHex(uint8Array) {
  ...
  while (uint8Array.length < 32) {
    uint8Array.unshift(0);
  }

  for (let i = 0; i < uint8Array.length; i += 4) {
    const value = (uint8Array[i] << 24) | (uint8Array[i + 1] << 16) | (uint8Array[i + 2] << 8) | uint8Array[i + 3];
    hexString += hex32(value);
  }
...
async function processNonceRange(task, startNonce, endNonce) {
  ...
  const mainFactorArray = bigintToUint8Array(BigInt(task.mainFactor));
  const shareFactorArray = bigintToUint8Array(BigInt(task.shareFactor));
...
function calculateHashNew(index, previousHash, data, nonce, timestamp, minerId) {
  const input = `${index}-${previousHash}-${data}-${nonce}-${timestamp}-${minerId}`;
  let buffer = createHash().update(input).digest();
  return buffer;
}
